...
for a in range(0, 20):
    ...
    # waste_per_person_per_month = 0.8
    # waste_per_person_per_year = waste_per_person_per_month * 12
    waste_per_person_per_year = 3.7 * (a * 0.05)
    ...
    # Constraint: (4)
    model.addConstrs(
        gp.quicksum(x[t, i, j, p] for i in generation_points) + inventory[t - 1, j, p]
    ...
    model.addConstrs(
        gp.quicksum(
            w[t, j, f, p] for t in periods for j in collection_centers for t in periods
        )
    ...
    file.write(
        # "\n" + str(model.ObjVal)
        f"\n{a*0.05*100} {round(model.ObjVal,2)} {round(TC_gp_cc.getValue(),2)} {round(TC_cc_firms.getValue(),2)} {round(Costs_of_establising_cc.getValue(),2)} {round(Costs_of_inventory.getValue(),2)} {round(model.Runtime,2)}"
    )
    file.close()
    model.dispose()
